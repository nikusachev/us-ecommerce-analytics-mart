...
Исходные CSV-файлы не включены в репозиторий из-за размера данных.  
Датасет доступен на Kaggle: [E-Commerce U.S Dataset (2019~2025, 1 Million rows)](https://www.kaggle.com/datasets/limjeongeun/synthetic-u-s-e-commerce-dataset-1m-orders).

Файл Power BI `.pbix` не включён в репозиторий, потому что превышает лимит GitHub 100 MB. Скриншоты дашборда доступны в папке `screenshots/`.
...
В проекте были приняты несколько важных решений по модели данных и расчёту метрик.

* Я разделил `mart.fct_orders` и `mart.fct_order_items`, потому что у них разный grain: `fct_orders` хранит данные на уровне заказа, а `fct_order_items` - на уровне товарной позиции.
* Перед построением order-level витрины я агрегировал `order_items` до уровня `order_id`, чтобы не размножать строки заказов при JOIN.
* Таблица `order_payments` также была агрегирована до уровня `order_id`, потому что один заказ может иметь несколько платежных строк.
* Revenue-метрики считаются только по доставленным заказам (`is_delivered = 1`), а отменённые заказы используются отдельно для анализа cancellation rate.
* Для customer analytics, repeat customers и retention используется `customer_unique_id`, потому что именно он отражает одного клиента в разных заказах.
* Для cohort retention промежуточный grain был выбран как `1 customer_unique_id + 1 active month`, потому что retention считается по клиентам, а не по количеству заказов.
* В Power BI я не связывал напрямую `fct_orders` и `fct_order_items`, потому что это две фактические таблицы с разным grain. Вместо этого обе таблицы связаны через общую календарную таблицу `mart.dim_date`.
* Полная cohort matrix содержала больше 80 месяцев, поэтому в дашборде retention был ограничен первыми 12 месяцами для читаемости.
* Геоданные были подготовлены на staging-слое, но региональная аналитика оставлена как будущее улучшения, чтобы первая версия дашборда оставалась сфокусированной на выручке, категориях, клиентах и retention.

## Сложности и решения

В процессе проекта возникло несколько аналитических и технических сложностей.

* В `raw.geolocation` было несколько строк на один `zip_code_prefix`, поэтому таблицу нельзя было напрямую JOIN-ить к клиентам или продавцам без предварительной агрегации.
* В `raw.order_payments` могло быть несколько платежных строк на один заказ, поэтому платежи были агрегированы до уровня заказа.
* Для order-level витрины было важно не соединять напрямую `orders`, `order_items` и `order_payments`, потому что это привело бы к размножению строк и завышению revenue-метрик.
* При построении cohort retention стало важно перейти от order-level логики к customer-month логике: один клиент в одном активном месяце должен считаться один раз, даже если он сделал несколько заказов.
* Полная retention-матрица на 80+ месяцев оказалась неудобной для Power BI, поэтому в дашборде показаны первые 12 месяцев после первой покупки.
* Файл Power BI `.pbix` превысил лимит GitHub 100 MB, поэтому в репозиторий были добавлены скриншоты дашборда и документация, а сам `.pbix` файл не включён.
